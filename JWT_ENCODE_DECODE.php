<?php
	...
	Class JwtAuth{
		...
		private function genPayload($email, $username, $user_id){ //dati i parametri genera il payload da dare al token generator
			$payload = array(
				"email" => $email,
				"username" => $username,
				"id_user" => $user_id
		...
		private function genToken($payload){ //dato il payload genererà il token
			$token = JWT::encode($payload,$this->secret_key);
			return $token;
		}


		public function encode($email, $username, $user_id){ //funzione utilizzabile dall'utente
		...
		private function decodeToken($token){ //ottiene stringa contenente tutti i parametri dal token. AGGIUNGERE CONTROLLO SUL DECODE DEL TOKEN
