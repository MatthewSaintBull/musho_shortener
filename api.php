...
if (isset($postdata)) { //se ci sono i requisiti nella get
    ...
    $short_code = $shortener->short($url); //genera un hash nuovo e univoco
    ...
    if ($url != "") { //se l'url ottenuta non è vuota
        $bulk->insert(['id' => $short_code, 'name' => $name, 'id_user' => $id_user, 'url_full' => $url, 'url_short' => $short_url]);  //esegue query per inserire in db
        $writeConcern = new MongoDB\Driver\WriteConcern(MongoDB\Driver\WriteConcern::MAJORITY, 1000);
        $result = $manager->executeBulkWrite('musho.url', $bulk, $writeConcern); //esegue la query
    } else {
